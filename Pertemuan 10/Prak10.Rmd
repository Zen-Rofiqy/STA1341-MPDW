...
```{r}
model_terbaik <- function(model, rowname = NA_character_) {
  if (!all(is.na(model))) {
    model_df <- data.frame(
      Model = paste("ARIMA(", 
    ...
    model_df$Keterangan <- KeteranganFungsi(model_df$Model)
  return(model_df)
...
drift1 <- bind_rows(
  model_terbaik(model_aapl.drift1, 'AAPL'),
  model_terbaik(model_amzn.drift1, 'AMZN'),
  model_terbaik(model_goog.drift1, 'GOOG'),
  model_terbaik(model_meta.drift1, 'META'),
  model_terbaik(model_msft.drift1, 'MSFT'),
  model_terbaik(model_nflx.drift1, 'NFLX'),
  model_terbaik(model_nvda.drift1, 'NVDA'),
) %>%
  mutate(across(where(is.numeric), ~round(., 2)))
