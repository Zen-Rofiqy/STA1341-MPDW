...
model_terbaik <- function(model) {
  data.frame(
    Model = ifelse(!is.na(model), 
                   paste("ARIMA(", paste(as.character(model[["call"]][["order"]][-1]), collapse = ","), ")", sep = ""),
                   NA_character_),
    AIC = ifelse(!is.na(model), model[["aicc"]], NA_real_)
  )
}
...
drift1 <- bind_rows(
  ...
  model_terbaik(model_goog.drift1),
  model_terbaik(model_meta.drift1),
  model_terbaik(model_msft.drift1),
  model_terbaik(model_nflx.drift1),
  model_terbaik(model_nvda.drift1),
) %>%
  mutate(across(where(is.numeric), ~round(., 2)))
