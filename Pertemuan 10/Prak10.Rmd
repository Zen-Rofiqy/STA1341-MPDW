...
Gambar di atas menunjukkan bahwa hanya `NFLX` yang memuat *rounded value* ($\lambda$) di dalam selangnya. Sementara selainnya tidak memuatnya.
...
trans <- function(lambda) {
  lambda <- lamb[sapply(lamb, function(y) which.min(abs(y - lambda))) ]
  case_when(
    lambda == -2   ~ "1/x^2",
    lambda == -1   ~ "1/x",
    lambda == -0.5 ~ "1/sqrt(x)",
