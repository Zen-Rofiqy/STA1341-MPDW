...
ggplot() + 
  #Label Data Asli 
  annotate( "rect", alpha=.1, fill="seagreen",
  ...
  annotate( "text", color="seagreen",
    ...
    label = paste0(nrow(train_nvda)," Hari"), size=10) + 
  ...
  annotate( "text", color="violetred",
            x = as.Date(data2$Date[ceiling( .89*length(data2$Date)/7)]) , 
            y = max(data2$`Adj Close`) * .85, 
    label = paste0(nrow(test_nvda)," Hari"), size=10) +
