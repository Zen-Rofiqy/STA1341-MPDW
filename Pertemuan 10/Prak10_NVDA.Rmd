...
#Ekstras
  # Titik Terendah
  geom_richtext(
    data = data.frame(x = as.numeric(min_date), 
                      y = max(`Adj Close`)*30/100, 
                      label = paste0("Titik Terendah\n",
                                     "(",round(min_perc, 2), "% Data)\n",
                                     "   ",min_date)),
    aes(x, y, label = label), size = 7, color = "grey30", 
    fill = "white", box.color = "grey30", parse = TRUE, 
    vjust = -1.5, hjust = 0
  ) +
  # Garis Panah
  geom_segment(aes(x = as.numeric(min_date), xend = as.numeric(min_date), 
                   y = 0, yend = max(`Adj Close`)*30/100),
                arrow = arrow(type = "closed", 
                              length = unit(0.1, "inches")), 
                lineend = "round", color = "grey30", size = 1.5) +
  
  # Titik Tertinggi
  geom_richtext(
    data = data.frame(x = as.numeric(max_date), 
                      y = max(`Adj Close`)*30/100, 
                      label = paste0("Titik Tertinggi\n",
                                     "(",round(max_perc, 2), "% Data)\n",
                                     "   ",max_date)),
    aes(x, y, label = label), size = 7, color = "grey30", 
    fill = "white", box.color = "grey30", parse = TRUE, 
    vjust = -1.5, hjust = 0
  ...
  # Garis Panah
  geom_segment(aes(x = as.numeric(max_date), xend = as.numeric(max_date), 
                   y = 0, yend = max(`Adj Close`)*30/100),
                arrow = arrow(type = "closed", 
                              length = unit(0.1, "inches")), 
                lineend = "round", color = "grey30", size = 1.5)
...
ggplot() + 
  ...
  geom_text(data = tail(train_nvda, 1), 
            aes(x = Date, y = max(`Adj Close`), label = 
                  paste0(max(train_nvda$Date) )), 
