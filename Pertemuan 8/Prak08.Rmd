...
Gambar di atas menunjukkan nilai *rounded value* ($\lambda$) optimum sebesar $0.11$ dan pada selang kepercayaan $95\%$ nilai memiliki batas bawah $0.67$ dan batas atas $0.88$. Selang tersebut tidak memuat nilai satu sehingga dapat dikatakan bahwa data saham Apple tidak stasioner dalam ragam.
...
Berdasarkan plot tersebut, terlihat bahwa plot ACF cuts off pada lag ke 0, lag 13, dan lag 14. Hal ini menandakan data sudah stasioner dalam rataan dan ketidakstasioneran data telah berhasil tertangani.
...
Berdasarkan plot tersebut, terlihat bahwa plot ACF cuts off pada lag ke 0, lag 13, dan lag 14. Sehingga model tidak dapat di identifikasi dengan plot ACF.
...
![](images/Screenshot 2023-10-29 231401.png){width="327"}

Nilai *rounded value* ($\lambda$) optimum sebesar $0.11$. Sehingga akan dipilih Lambda yang nilainya dekat yakni sebesar $0$ dengan transformasi $\log(\mathbf{Y})$.
...
y <- log(train_aapl.ts)
fit <- ets(y)
fc <- forecast(fit, h=h, level=0.95)
...
fit <- ets(y)
#---FORECAST---#
ramalan_aapl.da <- forecast::forecast(fit, h = h) 
# Transformasi Balik Hasil Ramal
ramalan_aapl.da$mean <- exp(ramalan_aapl.da$mean)
ramalan_aapl.da$lower <- exp(ramalan_aapl.da$lower)
ramalan_aapl.da$upper <- exp(ramalan_aapl.da$upper)
ramalan_aapl.da$x <- train_aapl.ts
...
Berdasarkan uji ADF tersebut, didapat *p-value* sebesar $0.11159$ yang lebih besar dari taraf nyata $5\%$ sehingga **tak tolak** $H_0$ dan menandakan bahwa data **tidak stasioner dalam rataan**. Hal ini sesuai dengan hasil eksplorasi menggunakan plot time series dan plot ACF, sehingga ketidakstasioneran model kedepannya harus ditangani.
...
Berdasarkan uji Ljung-Box terhadap sisaan kuadrat tersebut, didapat *p-value* sebesar $0.91155$ yang lebih besar dari taraf nyata $5\%$ sehingga **tak tolak** $H_0$ dan menandakan bahwa **ragam sisaan homogen**.
