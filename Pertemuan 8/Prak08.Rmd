...
Gambar di atas menunjukkan nilai *rounded value* ($\lambda$) optimum sebesar $1.68$ dan pada selang kepercayaan $95\%$ nilai memiliki batas bawah $0.64$ dan batas atas $2.72$. Selang tersebut memuat nilai satu sehingga dapat dikatakan bahwa data saham Apple stasioner dalam ragam.
...
Nilai *rounded value* ($\lambda$) optimum sebesar $1.68 \ne 0$. Sehingga akan menggunakan transformasi$\frac{x^\lambda-1}{\lambda}$ .

```{r}
train_aapl.ts <- (train_aapl.ts^lambda_aapl-1)/lambda_aapl
bc <- boxcox(train_aapl.ts ~ index)
```
...
![](images/Screenshot%202023-10-29%20231401.png){width="327"}
