...
h <- nrow(data2)/7 - round(nrow(data2)/7 *80/100)
```
...
```{r}
perbandingan_aapl.da <- matrix(data=c(head(test_aapl.ts, n=h), hasil[-1]),
                     nrow = h, ncol = 2)
...
```{r}
akurasi.aapl <- accuracy(ts(hasil[-1]), head(test_aapl.ts, n=h)) %>%
  as.data.frame() %>%
...
```{r}
perbandingan_amzn.da <- matrix(data=c(head(test_amzn.ts, n=h), hasil[-1]),
                     nrow = h, ncol = 2)
...
```{r}
akurasi.amzn <- accuracy(ts(hasil[-1]), head(test_amzn.ts, n=h)) %>%
  as.data.frame() %>%
...
```{r}
perbandingan_goog.da <- matrix(data=c(head(test_goog.ts, n=h), hasil[-1]),
                     nrow = h, ncol = 2)
...
```{r}
akurasi.goog <- accuracy(ts(hasil[-1]), head(test_goog.ts, n=h)) %>%
  as.data.frame() %>%
...
```{r}
perbandingan_meta.da <- matrix(data=c(head(test_meta.ts, n=h), hasil[-1]),
                     nrow = h, ncol = 2)
...
```{r}
akurasi.meta <- accuracy(ts(hasil[-1]), head(test_meta.ts, n=h)) %>%
  as.data.frame() %>%
...
```{r}
perbandingan_msft.da <- matrix(data=c(head(test_msft.ts, n=h), hasil[-1]),
                     nrow = h, ncol = 2)
...
```{r}
akurasi.msft <- accuracy(ts(hasil[-1]), head(test_msft.ts, n=h)) %>%
  as.data.frame() %>%
...
```{r}
perbandingan_nflx.da <- matrix(data=c(head(test_nflx.ts, n=h), hasil[-1]),
                     nrow = h, ncol = 2)
...
```{r}
akurasi.nflx <- accuracy(ts(hasil[-1]), head(test_nflx.ts, n=h)) %>%
  as.data.frame() %>%
...
```{r}
perbandingan_nvda.da <- matrix(data=c(head(test_nvda.ts, n=h), hasil[-1]),
                     nrow = h, ncol = 2)
...
```{r}
akurasi.nvda <- accuracy(ts(hasil[-1]), head(test_nvda.ts, n=h)) %>%
  as.data.frame() %>%
...
data3 <- data.frame(
  Date = data2 %>% select(Date) %>%
    filter(Date <= 
           as.Date(data2$Date[(ceiling(0.8 * nrow(data2)/7))]) + h) ,
  `Adj Close` = c(as.vector(train_aapl.ts), ramal_aapl,
  ...
  Name = rep(c("AAPL", "AMZN", "GOOG", "META", "MSFT", "NFLX", "NVDA"), 
              each = length(train_aapl.ts) + h + 1)
)
colnames(data3) <- c("Date", "Adj Close", "Name")
```

```{r dpi=300, fig1, fig.height = 12, fig.width = 27, fig.align = "center", warning=TRUE, message = FALSE}
ggplot(data3, aes(x = Date, y = `Adj Close`, color = Name)) +
  ...
  annotate( "text", color="seagreen",
            x = as.Date(data2$Date[ceiling(length(data2$Date)/7/2)]), 
            y = max(data2$`Adj Close`) * 0.95, 
    label = "Data Asli", size=10) + 
  
  #Label Data Ramal
  annotate( "rect", alpha=0.1, fill="violetred",
            xmin=as.Date(max(data2$Date)), 
  ...
  labs(x = "\nPeriode (Tahun)", y='Harga Saham (USD)',
       ...
       subtitle = "Peramalan Saham MAANG Selama h Hari Kedepan\n") +
  # Label / legend
  geom_text_repel(
  ...
  geom_vline( #Buat garis batas data
    xintercept = as.Date(max(data2$Date)) , 
             linetype = "dotted", color = "black", linewidth = 2)
```
