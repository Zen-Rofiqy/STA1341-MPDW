...
#Pnggabungan data
data_plot <- data.frame(
  Periode = data2 %>% select(Date) %>%
    filter(Date < as.Date(data2$Date[ceiling(0.8 * length(data2$Date)/7)])) ,
  Harga = c(as.vector(train_aapl.ts), as.vector(train_amzn.ts), 
            as.vector(train_goog.ts), as.vector(train_meta.ts), 
            as.vector(train_msft.ts), as.vector(train_nflx.ts), 
            as.vector(train_nvda.ts) ),
  Saham = rep(c("AAPL", "AMZN", "GOOG", "META", "MSFT", "NFLX", "NVDA"), 
              each = length(train_aapl.ts))
)

# Buat plot menggunakan ggplot2 dengan facet_wrap
ggplot(data_plot, aes(x = Periode, y = Harga, color = Saham)) +
