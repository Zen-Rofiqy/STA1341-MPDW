...
fill_missing_data <- function(name) {
  ...
  full_date_range <- seq(min(data$Date), max(data$Date), by = "days")
...
filled_data_list <- map(unique(data$Name), fill_missing_data)
...
data <- data.frame(final_data$Date, final_data$Name, data)
```
