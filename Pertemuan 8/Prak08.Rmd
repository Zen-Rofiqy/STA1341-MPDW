...
ggplot(data2, aes(x=Date, y=`Adj Close`, color=Name, alpha=Name)) + #Data
  ...
  annotate( "text", 
            ...
            y = data2$`Adj Close` * 0.9, 
    label = "Data Latih", size=6) + 
  ...
  annotate( "text", 
            x = (as.Date(max(data2$Date)) - 
                 as.Date(data2$Date[ceiling(0.8 * 
                                    length(data2$Date)/7)]) )/2 , 
            y = data2$`Adj Close` * 0.9, 
    label = "Data Uji", size=6)
            
chart

#Export Chart
ggsave("0_Data Uji vs Data Latih.png", chart, path = export.chart,
        dpi = 300, height = 12, width = 23)
```
