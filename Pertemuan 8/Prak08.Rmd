...
Dapat dilihat bahwa rata-rata harga saham Apple diramalkan akan cenderung menaik setiap periodenya.
...
Nilai *rounded value* ($\lambda$) optimum sebesar $-0.22$. Nilai ini mendekati nilai $-0.5$, Sehingga akan menggunakan transformasi $1/\sqrt{x}$.

```{r}
train_amzn.ts.new <- 1/sqrt(train_amzn.ts)
...
Berdasarkan pendugaan parameter di atas, nilai AIC terkecil $-3061.78$ dimiliki oleh model `ARIMA(0,1,1)` dan seluruh parameternya signifikan sehingga model yang dipilih adalah model `ARIMA(0,1,1)`.
...
Dapat dilihat bahwa rata-rata harga saham amazon diramalkan akan terus menaik setiap periodenya.
...
Didapatkan nilai MAPE sebesar $12.04369\%$ (lebih dari $10\%$) yang menandakan bahwa hasil peramalan dari model `ARIMA(0,1,1)` sudah baik.
...
Berdasarkan pendugaan parameter di atas, nilai AIC terkecil $2158.74$ dimiliki oleh model `ARIMA(0,1,1)` dan seluruh parameternya signifikan sehingga model yang dipilih adalah model `ARIMA(0,1,1)`.
