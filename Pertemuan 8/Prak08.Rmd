...
data3 <- data.frame(
  ...
  Saham = rep(c("AAPL", "AMZN", "GOOG", "META", "MSFT", "NFLX", "NVDA"), 
              each = length(train_aapl.ts) + 54 + 1)
)
```

```{r dpi=300, fig1, fig.height = 12, fig.width = 23, fig.align = "center", warning=TRUE, message = FALSE}
# Buat plot menggunakan ggplot2 dengan facet_wrap
ggplot(data3, aes(x = Date, y = Harga, color = Saham)) +
  #Label Data Asli 
  annotate( "rect", alpha=0.1, fill="seagreen",
            xmin=as.Date(min(data2$Date)), 
            xmax=as.Date(max(data2$Date)),
  ...
  annotate( "text", color="seagreen",
            x = as.Date(data2$Date[ceiling(max(data2$Date)/2)]), 
            y = max(data2$`Adj Close`) * 0.95, 
  ...
  labs(x = "\nPeriode (Tahun)", y='Harga Saham (USD)',
       ...
       subtitle = "Peramalan Saham MAANG Selama 54 Hari Kedepan\n") +
  # Label / legend
  geom_text_repel(
  ...
  #Axis
    ...
    limits = c(as.Date(min(data3$Date)), 
               as.Date(data3$Date[ceiling(max(data3$Date) * 0.1)]) )
  ) +
    scale_y_continuous( #Sumbu y
    labels = scales::dollar_format(prefix = "$") #tambahin dolar
  ) +
