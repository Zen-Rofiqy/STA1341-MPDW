...
for(i in 1:lag){
  ...
  aic <- summary(arch.fit)[["ics"]][["AIC"]]
    ...
    coef <- coef(arch.fit)
    sig <- sum(2 * (1 - pt(abs(arch.fit@fit[["tval"]]), df)) < .05) / length(coef)
  
    sig.param <- names(arch.fit@fit[["tval"]][2 * 
                          (1 - pt(abs(arch.fit@fit[["tval"]]), df)) < 0.05])
    non_sig.param <- names(arch.fit@fit[["tval"]][2 * 
                          (1 - pt(abs(arch.fit@fit[["tval"]]), df)) >= 0.05])
    model <- paste0("ARCH(",i,")")
