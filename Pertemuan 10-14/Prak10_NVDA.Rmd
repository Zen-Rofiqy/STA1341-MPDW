...
create_spec <- function(i, j, arima_order) {
  return(ugarchspec(
...
arch.garch <- function(data, pp, qq = 0, arima.order = c(0, 0, 0)) {
  ...
  for (i in 1:pp) {
    if (qq == 0) {
      # ARCH model
      formula <- as.formula(paste("~ garch(", i, ",0)", sep = ""))
      archSpec <- create_spec(i, 0, arima.order)
    } else {
      for (j in 1:qq) {
        # GARCH model
        formula <- as.formula(paste("~ garch(", i, ",", j, ")", sep = ""))
        archSpec <- create_spec(i, j, arima.order)
      }
    }
...
```{r}
# Contoh penggunaan
result <- arch.garch(data = train_nvda.diff, pp = 12, qq = 0, arima.order = c(0, 1, 1))
datatable(result$arch.model)
