...
3.  **Heteroskedasticity:** Ini merujuk pada sifat ketidakseragaman dalam variabilitas. Dalam konteks ARCH, heteroskedasticity menunjukkan bahwa variabilitas suatu variabel tidak konstan, tetapi bervariasi sepanjang waktu berdasarkan informasi kondisional.
...
Untuk memahami model ARCH, mari kita mulai dengan mendiskusikan konsep rata-rata dan varians kondisional dan tak bersyarat dari istilah kesalahan dalam data deret waktu. Pertama, pertimbangkan model auto-regresif **AR(1)**.

### **Uji Efek ARCH**

```{r}
lag <- 12
arch_effect <- data.frame(p.value = numeric(0), 
                          Keterangan = character(0))
for(i in 1:lag){
  p.value <- ArchTest(sisaan_nvda.da, lags=i, demean=TRUE)[["p.value"]]
  arch_effect <- rbind(arch_effect, 
  ...
  rownames(arch_effect)[i] <- paste0("Lag ", i)
}
datatable(arch_effect, filter = 'top', 
          options = list(pageLength = 6))
```

> $H_0$ : Lag Tidak signifikan
>
> $H_1$ : Lag Signifikan

Dari Lag 1 sampai Lag 12 yang ditest, Terlihat bahwa Lag 2 memiliki *p-value* sebesar $0.08$ yang lebih besar dari taraf nyata $5\%$, sehingga **tolak** $H_0$. Artinya Lag 2 Tidak sinifikan, selain itu semua lag nya signifikan. Karena pada saat Lag 2 saja sudah tidak signifikan maka **tidak perlu** dilakukan model GARCH.

```{r}
ArchTest(sisaan_nvda.da)

if (ArchTest(sisaan_nvda.da)[["p.value"]] < 0.05) {
  cat("Karena p-value < 0.05, Maka Ada efek ARCH")
} else {
  cat("Karena p-value > 0.05, Maka Tidak ada efek ARCH")
}
...
Uji LM menunjukkan *p-value* kurang dari taraf nyata $5\%$, yang mengindikasikan bahwa $H_0$ **ditolak**. Oleh karena itu, Hasil `NVDA` menunjukkan adanya **efek ARCH**. Jadi walaupun Lag 2 tidak signifikan, namun Lag lainnya signifikan dan menurut uji ini juga menunjukkan adanya efek ARCH. Maka bisa dilanjutkan ke model GARCH.

### Model ARCH

Ya untuk berjaga-jaga dan kami ingin membandingkan mana hasil yang terbaik, kami juga mencobakan model ARCH nya.
