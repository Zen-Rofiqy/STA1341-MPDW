...
arch.garch <- function(data, pp, qq = 0, arma.order = c(0, 0)) {
  ...
  for (i in 1:pp) {
    for (j in 0:qq) {
      # Buat spesifikasi model GARCH
      spec <- ugarchspec(
        variance.model = list(model = "sGARCH", garchOrder = ifelse(j == 0, c(i, 0), c(i, j))),
        mean.model = list(armaOrder = arma.order)
      )

      # Buat model GARCH
      archFit <- ugarchfit(spec = spec, data = data, solver = "hybrid")
      ...
      # Perhitungan AIC
      aic <- infocriteria(archFit)[[1]]
      # P-value
      p_values <- archFit@fit$robust.matcoef[, 4]
      # Persentase banyaknya parameter yang signifikan
      ...
      # Identifikasi best model
      if (is.null(best_model) || sig > arch.model$Signif[best_model] || (sig == arch.model$Signif[best_model] && aic < arch.model$AIC[best_model])) {
        best_model <- nrow(arch.model)
      }
    }
  }
  ...
  result <- list(
    ...
    best_model = arch.model$Model[best_model]
  )
  return(result)
}
...
```{r message=FALSE, warning=FALSE}
model.garch <- arch.garch(data = train_nvda.diff, pp = 12, qq = 12, arma.order = arma.order)
...
```{r fig.align="center", fig.height=9, fig.width=16, message=FALSE, warning=FALSE, dpi=300}
plot(model.arch_nvda, which="all")
```
...
## **Peramalan dengan Model ARIMA(0,1,1)+ARCH(1)**

```{r}
forecast3 <- ugarchforecast(garch7afold3, ihsg.dif3, 8)
forecast3

plot(forecast3, which= 1)

pt_3 <- ihsg.train3[254]

hasil.forc.Diff3 <- forecast3@forecast[["seriesFor"]]
hasil3 <- diffinv(hasil.forc.Diff3, differences = 1) + pt_3

perbandingan3 <- data.frame("Aktual"= ihsg.test3[1:8],
                            "Ramalan" = hasil3[-1],
                            "Tanggal" = tanggal.fold3)

forecast::accuracy(ts(hasil3[-1]),ts(ihsg.test3[1:8]))
...
#--Forecast--
  ...
  #ARCH
  ramalan_nvda.arch = ugarchforecast(model.arch_nvda, data = train_nvda.diff, 
                        n.ahead = nrow(test_nvda))
  data.ramal_nvda.arch <- ramalan_nvda.arch@forecast[["seriesFor"]]
```
...
#--Diff inv--
  ...
  #ARCH
  ramal_nvda.arch <- diffinv(data.ramal_nvda.arch, differences=1) +
    train_nvda.ts.new[length(train_nvda.ts)] #nilai akhir data latih
...
#--Transformasi Balik--
  ...
  #ARCH
  ramal_nvda.arch <- 1/(ramal_nvda.arch)^2

#Data Frame
ramal <- data.frame(
  Date = data2 %>% dplyr::select(Date)  ,
  `ARIMA` = c(as.vector(train_nvda.ts), ramal_nvda[-1] ),
  `ARCH` = c(as.vector(train_nvda.ts), ramal_nvda.arch[-1] ),
  Name = rep(c("NVDA"), 
              each = nrow(train_nvda) + nrow(test_nvda) )
)
colnames(ramal) <- c("Date", "ARIMA", "ARCH", "Name")
```
...
#Time Series
  ...
  geom_line(data = ramal[nrow(train_nvda):nrow(ramal),], linewidth=2,
            aes(x = Date, y = `ARCH`, col = "NVDA")) +
  #Data Test
  geom_line(data = test_nvda, linewidth=2,
            aes(x = Date, y = `Adj Close`, col = "test_nvda")) +
...
chart
#Export Chart
ggsave("09_banding_ramal_ARIMA-ARCH.png", chart, path = export.chart,
        dpi = 300, height = 12, width = 27)
```
...
perbandingan_nvda.da <- matrix(data=c(head(test_nvda.ts, n=nrow(test_nvda)), 
                                      ramal_nvda[-1],
                                      ramal_nvda.arch[-1]),
                               nrow = nrow(test_nvda), ncol = 3)
colnames(perbandingan_nvda.da) <- c("Aktual","Forecast ARIMA", "Forecast ARCH")
...
#Akurasi Model ARCH
akurasi.nvda.arch <- accuracy(ts(ramal_nvda.arch[-1]), 
                         head(test_nvda.ts, n=nrow(test_nvda) )) %>%
  as.data.frame() %>%
  cbind(Model = model.arch$best_model) %>% 
  relocate(Model, .before = 1)
row.names(akurasi.nvda.arch) <- "ARCH"
```

```{r}
akurasi <- rbind(akurasi.nvda, akurasi.nvda.arch) %>%
