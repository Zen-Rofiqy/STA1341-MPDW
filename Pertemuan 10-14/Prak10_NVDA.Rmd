...
# Data Ramal
  geom_segment(aes(x = as.Date(max(data2$Date))-40, 
                   xend = as.Date(max(data2$Date))-50, 
                   y = max(nvda$`Adj Close`)* .9, 
                   yend = max(nvda$`Adj Close`)*.9), 
               arrow = arrow(type = "closed", length = unit(.1, "inches")), 
               lineend = "round", color = "#D02A49", size=1.5) +
  geom_richtext(
    data = data.frame(x = as.Date(max(data2$Date))-40, 
                      y = max(nvda$`Adj Close`)*.9, 
                      label = "Data Ramal" ),
    aes(x, y, label = label), size = 8, color = "white", 
    fill = "#D02A49", box.color = "white", parse = TRUE
