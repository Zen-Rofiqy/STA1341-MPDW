...
```{r fig.align="center", fig.height=12, fig.width=27, message=FALSE, warning=FALSE, dpi=300}
install_load('ggtext')
...
```{r dpi=300,  fig.height = 12, fig.width = 28, fig.align = "center", warning=FALSE, message = FALSE}
chart <- 
ggplot() +
# Akhir data
  geom_vline( #Buat garis batas data
...
#Time Series
  ...
  scale_colour_manual(values = c("Trans"=col.nvda[1])) +
  theme.ts + #THeme
  labs(x = "\nPeriode (Tahun)", y='Harga Saham (USD)',
       title = "Plot Transformasi NVDA",
       subtitle = "Pola deret waktu hasil Transformasi\n") +
  #Axis
    coord_cartesian(clip = "off"
  ) +
    scale_x_date( #Sumbu x
  ...
  ylim(min(ragam$Trans), max(ragam$Trans)) +
...
#Tanggal akhir
  geom_segment(aes(x = as.Date(max(train_nvda$Date)), 
                   xend = as.Date(max(train_nvda$Date)), 
                   y = -Inf, 
                   yend = max(ragam$Trans)*.15), 
               arrow = arrow(type = "closed", length = unit(.1, "inches")), 
               lineend = "round", color = "gray60", size=1.5) +
  geom_richtext(
    data = data.frame(x = as.Date(max(train_nvda$Date)), 
                      y = max(ragam$Trans)*.15, 
                      label = max(train_nvda$Date) ),
    aes(x, y, label = label), size = 8, color = "white", 
...
chart
#Export Chart
ggsave("03_Stas_4_Transform_1.png", chart, path = export.chart,
        dpi = 300, height = 12, width = 28)
```
...
chart
#Export Chart
ggsave("03_Stas_4_Transform_2.png", chart, path = export.chart,
        dpi = 300, height = 12, width = 27)
```
...
```{r, include=FALSE, echo=FALSE}
#Export Chart
png(paste(export.chart, "/03_Stas_5_Diff.png", sep=""),
...
```{r, include=FALSE, echo=FALSE}
#Export Chart
png(paste(export.chart, "/03_Stas_6_ACF.png", sep=""),
    height = 4*300, width = 7.5*300, res=300)
...
```{r, include=FALSE, echo=FALSE}
#Export Chart
png(paste(export.chart, "/03_Stas_7_ACF-PACF.png", sep=""),
...
![](images/Screenshot%202023-12-02%20060410.png){width="296"}

Dalam hal ini model tentatif yang terbentuk adalah ARIMA(0,1,0), ARIMA(1,1,1), ARIMA(2,1,2), ARIMA(3,1,3), dst.. Namun karena kemungkinannya sangat banyak, maka akan digunakan function agar menyingkat proses.
    ...
    for (q in 1:q_max) {
      ...
      if (!is.na(eacf_result$symbol[p + 1, q ]) && 
        ...
        if (eacf_result$symbol[p + 1, q ] == "o" && 
          ...
          model <- Arima(data, order=c(p,d,q-1), method="ML", 
                         include.drift=drift)
          aic <- AIC(model)
          ...
          models <- rbind(models, 
                    data.frame(Model = paste("ARIMA(", p, ",", d, ",", q-1, ")", 
                                             sep = ""), 
                               AIC = aic, 
...
Berdasarkan pendugaan parameter di atas, nilai AIC terkecil $-8917.36$ dimiliki oleh model `ARIMA(0,1,1)` dan seluruh parameternya signifikan sehingga model yang dipilih adalah model `ARIMA(0,1,1)`.
...
Berdasarkan pendugaan parameter di atas, nilai AIC terkecil $5063.29$ dimiliki oleh model `ARIMA(4,1,4)` dan seluruh parameternya signifikan sehingga model yang dipilih adalah model `ARIMA(4,1,4)`.
...
```{r}
datatable(head(model.tentaif_nvda, 3), filter = 'top', 
          options = list(pageLength = 3))
```

Terlihat bahwa hanya terdapat model `ARIMA(0,1,2)` dan `ARIMA(1,1,1)` tidak jauh lebih baik daripada model `ARIMA(0,1,1)` .
