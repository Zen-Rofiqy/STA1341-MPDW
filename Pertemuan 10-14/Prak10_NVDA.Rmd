...
arch.garch <- function(data, pp, qq = 0, arma.order = c(0, 0)) {
  ...
  for (i in 1:pp) {
    if (qq == 0) {
      ...
      archSpec <- create_spec(i, 0, arma.order)
    } else {
      for (j in 1:qq) {
        # GARCH model
...
model.arch_nvda <- ugarchfit(spec=archSpec, data=train_nvda.diff, solver = "hybrid")
coef(model.arch_nvda)
```
...
```{r dpi=300, fig.height = 6, fig.width = 12, fig.align = "center"}
plot.ts(ts(model.arch_nvda@fit$residuals), col=col.nvda[1], lty=1, 
        xlab="Periode (Hari)", ylab="Conditional Variance",
        lwd = 2, main="ARCH(1) NVDA", cex.main = 2)
```
