...
arima.order <- c(
  ...
  arima.best_model_nvda[["call"]][["order"]][[3]],
...
1.  **Grafik Harga atau Hasil Log:** Melihat perubahan harga atau hasil log dari waktu ke waktu dapat memberikan gambaran tentang tingkat volatilitas. Jika ada variasi besar dalam harga atau hasil log, ini menunjukkan keberadaan volatilitas.

2.  **Analisis Hasil Kuadrat:** Volatilitas sering kali terkait dengan variasi kuadrat dari hasil. Melakukan analisis terhadap hasil kuadrat atau volatilitas kondisional dapat membantu mengidentifikasi keberadaan volatilitas.

3.  **Analisis Hasil Mutlak:** Melihat hasil mutlak (absolute returns) juga dapat membantu dalam mendeteksi volatilitas, karena hasil mutlak mencerminkan besarnya perubahan harga tanpa memperhatikan arahnya.

```{r fig.align=, fig.height=5, fig.width=10, message=FALSE, warning=FALSE, dpi=300}
install_load('fGarch', 'PerformanceAnalytics', 'rugarch', 'xts', 'FinTS')

#calculate log returns and remove first NA value
Return.nvda <- Return.calculate(train_nvda.ts, method = "log")[-1]
volatility <- cbind(Return.nvda, Return.nvda^2, abs(Return.nvda))
colnames(volatility) <- c("Returns", "Returns^2", "abs(Returns)")
plot.zoo(volatility, main="NVDA Daily Returns", col=col.nvda[1], lwd=1.5)
```

```{r, include=FALSE, echo=FALSE}
#Export Chart
png(paste(export.chart, "05_Arch_1_volatility.png", sep=""),
    height = 5*300, width = 10*300, res=300)
...
```{r fig.align="center", fig.height=3.75, fig.width=11.25, message=FALSE, warning=FALSE, dpi=300}
# plot autocorrelations of returns, returns^2 and abs(returns)
par(mfrow=c(1,3))
stats::acf(Return.nvda, main="NVDA Returns", col=col.nvda[1], lwd=2,
           cex.main=10, ylim=c(-.2,.3))
stats::acf(Return.nvda^2, main="NVDA Returns^2", col=col.nvda[1], lwd=2,
           cex.main=10, ylim=c(-.2,.3))
stats::acf(abs(Return.nvda), main="NVDA abs(Returns)", col=col.nvda[1], lwd=2,
           cex.main=10, ylim=c(-.2,.3))
```

Terlihat bahwa baik dari hasil log, kuadrat, dan mutlak memperlihatkan **tidak adanya auto-korelasi** yang ditandai oleh plot tidak berpola *tails of slowly*. Kita dapat memastikan keberadaan autokorelasi nya dengan melakukan uji Ljung-Box pada hasil.

```{r, include=FALSE, echo=FALSE}
#Export Chart
png(paste(export.chart, "/05_Arch_2_acf.png", sep=""),
    height = 3.75*300, width = 11.25*300, res=300)
...
```{r}
corr <- data.frame(
  p.value = c(Box.test(coredata(Return.nvda), 
                       type = "Ljung", lag = 12)[["p.value"]],
              Box.test(coredata(Return.nvda^2), 
                       type = "Ljung", lag = 12)[["p.value"]],
              Box.test(coredata(abs(Return.nvda)), 
                       type = "Ljung", lag = 12)[["p.value"]]
              )
) 
corr$Keterangan <- ifelse(corr$p.value < 0.05, 
                              "Ada auto-korelasi", 
                              "Tidak ada auto-korelasi")

corr$p.value <- ifelse(round(corr$p.value, 2) == 0, 
                       sprintf("%.2e", corr$p.value),
                           round(corr$p.value, 2) )

rownames(corr) <- c("NVDA Returns", "NVDA Returns^2", "NVDA abs(Returns)")
datatable(corr, filter = 'top', 
          options = list(pageLength = 7))
```

> $H_0$ : Tidak ada Auto-korelasi
>
> $H_1$ : Ada Auto-korelasi

Terlihat bahwa hasil kuadrat dan hasil mutlak nilai *p-value* nya kurang dari taraf nyata $5\%$, sehingga **tolak** $H_0$. Artinya hasil kuadrat dan hasil mutlak memiliki auto-korelasi.
...
```{r}
ArchTest(Return.nvda)

if (ArchTest(Return.nvda)[["p.value"]] < 0.05) {
  cat("Karena p-value < 0.05, Maka Ada efek ARCH")
} else {
  cat("Karena p-value > 0.05, Maka Tidak ada efek ARCH")
}
```

Uji LM menunjukkan *p-value* kurang dari taraf nyata $5\%$, yang mengindikasikan bahwa $H_0$ **ditolak**. Oleh karena itu, Hasil `NVDA` menunjukkan adanya **efek ARCH**.
...
Referensi : [Measuring Volatility Using the ARCH Model](https://rpubs.com/cyobero/arch){.uri}
...
arch.garc <- function(data, pp, qq=0, arima.order=c(0,0,0) ){
...
for(i in 1:lag){
  formula <- as.formula(paste("~ garch(", i, ",0)", sep = ""))
  arch.fit <- garchFit(formula, data = train_nvda.diff, trace = FALSE)
  
  if (anyNA(coef(arch.fit)) || anyNA(arch.fit@fit[["tval"]])) {
    break
  }
  aic <- summary(arch.fit)[["ics"]][["AIC"]]
  
    # persentase banyaknya param yang signif
    coef <- coef(arch.fit)
    #Cek Pvalue yang < 0.05
    sig <- sum(arch.fit@fit$matcoef[,4] < .05) / length(coef)
  
    sig.param <- names(arch.fit@fit[["tval"]][arch.fit@fit$matcoef[,4] < 0.05])
    non_sig.param <- names(arch.fit@fit[["tval"]][arch.fit@fit$matcoef[,4] >= 0.05])
    
    model <- paste0("ARCH(",i,")")
  arch.model <- rbind(arch.model, 
      data.frame(Model = model,
                 Signif = sig, 
                 AIC = aic,
                 Sig.par = paste(sig.param, collapse = ", "),
                 Non_Sig.par = paste(non_sig.param, collapse = ", ")
                 ) )
...
arch.garch <- function(data, pp, qq = 0, arima.order = c(0, 0, 0)) {
  arch.model <- data.frame(Model = character(0),
                            Signif = numeric(0),
                            AIC = numeric(0),
                            Sig.par = character(0),
  ...
  create_spec <- function(i, j) {
    return(ugarchspec(
      variance.model = list(model = "sGARCH", garchOrder = c(i, j)),
      mean.model = list(armaOrder = arima.order)
    ))
  }
  ...
  for (i in 1:pp) {
    if (qq == 0) {
      # ARCH model
      formula <- as.formula(paste("~ garch(", i, ",0)", sep = ""))
      archSpec <- create_spec(i, 0)
    } else {
      for (j in 1:qq) {
        # GARCH model
        formula <- as.formula(paste("~ garch(", i, ",", j, ")", sep = ""))
        archSpec <- create_spec(i, j)
      }
    }
    ...
    aic <- summary(archFit)[["ics"]][["AIC"]]
    
    # persentase banyaknya param yang signif
    coef_values <- coef(archFit)
    
    # Cek Pvalue yang < 0.05
    sig <- sum(archFit@fit$matcoef[, 4] < 0.05) / length(coef_values)
    
    sig_param <- names(archFit@fit[["tval"]][archFit@fit$matcoef[, 4] < 0.05])
    non_sig_param <- names(archFit@fit[["tval"]][archFit@fit$matcoef[, 4] >= 0.05])
    
    model <- ifelse(qq == 0, paste0("ARCH(", i, ")"), paste0("GARCH(", i, ",", j, ")"))
  ...
  if (qq == 0) {
    ...
    return(list(arch.model = arch.model))
  }
}
...
```{r}
# Contoh penggunaan
result <- arch.garch(data = train_nvda.diff, pp = 3, qq = 2, arima.order = c(0, 1, 1))
print(result$arch.model)
print(paste("Best Model:", result$best_model))
```

```{r}
datatable(arch.model, filter = 'top', 
          options = list(pageLength = 6))
```
...
Setelah mencoba iterasi lag 1 sampai lag 12, terlihat bahwa hanya dapat menghasilkan 2 (artinya sisanya menghasilkan NaN) model yakni, **ARCH(1)** dan **ARCH(2)**. Dan diperoleh bahwa **model terbaik** yakni model **ARCH(1)** karena memiliki parameter yang signifikan paling banyak (berdasarkan persentasenya) dan juga nilai AIC yang terkecil.

```{r}
model.arch_nvda <- garchFit(~garch(1,0), data = train_nvda.diff, trace = F)
```

### Plot

```{r dpi=300, fig.height = 6, fig.width = 12, fig.align = "center"}
plot.ts(model.arch_nvda@h.t, lty=1, xlab="Periode (Hari)", ylab="Conditional Variance",
        col = col.nvda[1], lwd = 2,
        main="ARCH(1) NVDA", cex.main = 2)
```
...
```{r, include=FALSE, echo=FALSE}
#Export Chart
png(paste(export.chart, "/05_Arch(1)_NVDA.png", sep=""),
    height = 6*300, width = 12*300, res=300)
...
garchSpec11 <- ugarchspec(
  ...
  mean.model=list(armaOrder = arima.order) )
garchFit11 <- ugarchfit(spec=garchSpec11, data=train_nvda.diff)
coef(garchFit11)
```

```{r echo=FALSE, message=FALSE, warning=FALSE, results = FALSE}
infocriteria(garchFit11)[[1]] #AIC
garchFit11@fit$robust.matcoef[,4] #P.Value

garch.model <- data.frame(Model = character(0),
                          Signif = numeric(0),
                          AIC = numeric(0),
                          Sig.par = character(0),
                          Non_Sig.par = character(0))
...
for(p in 1:lag){
  for(q in 1:lag){
    formula <- as.formula(paste("~ garch(", p, ",0)", sep = ""))
    arch.fit <- garchFit(formula, data = train_nvda.diff, trace = FALSE)
    
    garchSpec <- ugarchspec(
      variance.model=list(model="sGARCH", garchOrder=c(p,q)),
      mean.model=list(armaOrder=c(0,1,1)) )
    garchFit <- ugarchfit(spec=garchSpec, data=train_nvda.diff)
...
```{r fig.align="center", fig.height=3.75, fig.width=7.5, message=FALSE, warning=FALSE, dpi=300}
res <- garchFit11@fit$residuals
res.ts <- ts(res)
plot.ts(res.ts, col=col.nvda[1])
```
...
garchFit12 <- ugarchfit(spec=garchSpec12, data=train_nvda.diff)
coef(garchFit12)
```
...
res2.ts<-ts(res2)
plot.ts(res2.ts, col=col.nvda[1])
```
...
garchSpec13 <- ugarchspec(
  variance.model=list(model="sGARCH",
                      garchOrder=c(1,3)),
mean.model=list(armaOrder=c(0,2)))
garchFit13 <- ugarchfit(spec=garchSpec13, data=train_nvda.diff)
coef(garchFit13)
```
...
```{r fig.align="center", fig.height=3.75, fig.width=7.5, message=FALSE, warning=FALSE, dpi=300}
res3<-garchFit13@fit$residuals
res3.ts<-ts(res3)
plot.ts(res3.ts, col=col.nvda[1])
```
...
garchSpec14 <- ugarchspec(
  variance.model=list(model="sGARCH",
                      garchOrder=c(1,4)),
mean.model=list(armaOrder=c(0,2)))
garchFit14 <- ugarchfit(spec=garchSpec14, data=train_nvda.diff)
coef(garchFit14)
```
...
```{r fig.align="center", fig.height=3.75, fig.width=7.5, message=FALSE, warning=FALSE, dpi=300}
res4<-garchFit14@fit$residuals
res4.ts<-ts(res4)
plot.ts(res4.ts, col=col.nvda[1])
```
...
garchSpec21 <- ugarchspec(
  variance.model=list(model="sGARCH",
                      garchOrder=c(2,1)),
mean.model=list(armaOrder=c(0,2)))
garchFit21 <- ugarchfit(spec=garchSpec21, data=train_nvda.diff)
coef(garchFit21)
```
...
res21.ts<-ts(res21)
plot.ts(res21.ts, col=col.nvda[1])
```

### UJI DIAGNOSTIK GARCH(1,2)

```{r message=FALSE, warning=FALSE}
nvda.garch12 <- garch(train_nvda.ts, c(1,2)) #ordo ARCH(1)
```

```{r}
summary(nvda.garch12)
```

```{r fig.align="center", fig.height=7.5, fig.width=7.5, message=FALSE, warning=FALSE, dpi=300}
#str(sbydarch)
plot(nvda.garch12, ask=FALSE) 
```
...
```{r}
# GARCH(1,2)
m.12 = garch(train_nvda.diff, order=c(1,2), trace =FALSE)
summary(m.12)
```

### **MODEL ARIMA(0,1,2)-GARCH(1,2)**
...
m.12<-ugarchfit(spec=model12,data=train_nvda.diff, out.sample = 100)
plot(m.12,which="all")
```
...
ramal <- data.frame(
  ...
  `Adj Close` = c(as.vector(train_nvda.ts), ramal_nvda[-1] ),
  Name = rep(c("NVDA"), 
...
#Data Uji
  geom_segment(aes(x = as.Date(max(data2$Date))-20, 
                   xend = as.Date(max(data2$Date))-20, 
                   y = max(nvda$`Adj Close`)* .85, 
                   yend = max(nvda$`Adj Close`)*.75), 
               arrow = arrow(type = "closed", length = unit(.1, "inches")), 
               lineend = "round", color = "#EDB64E", size=1.5) +
  geom_richtext(
    data = data.frame(x = as.Date(max(data2$Date))-20, 
                      y = max(nvda$`Adj Close`)*.75, 
                      label = "Data Uji" ),
    aes(x, y, label = label), size = 8, color = "white", 
    fill = "#EDB64E", box.color = "white", parse = TRUE
  ) +

# Data Ramal
  geom_segment(aes(x = as.Date(max(data2$Date))-45, 
                   xend = as.Date(max(data2$Date))-80, 
                   y = max(nvda$`Adj Close`)* 1.01, 
                   yend = max(nvda$`Adj Close`)*1.07), 
               arrow = arrow(type = "closed", length = unit(.1, "inches")), 
               lineend = "round", color = "#D02A49", size=1.5) +
  geom_richtext(
    data = data.frame(x = as.Date(max(data2$Date))-80, 
                      y = max(nvda$`Adj Close`)*1.07, 
                      label = "Data Ramal" ),
    aes(x, y, label = label), size = 8, color = "white", 
    fill = "#D02A49", box.color = "white", parse = TRUE
  ) +
