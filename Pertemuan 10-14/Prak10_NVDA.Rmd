...
archSpec <- ugarchspec(
  variance.model=list(model="sGARCH", garchOrder=c(1,0)),
  mean.model=list(armaOrder = arma.order, include.mean = TRUE) )
model.ramal <- ugarchfit(spec=archSpec, data=nvda.ts, solver="hybrid")
...
model.ramal = ugarchforecast(model.arch_nvda1, data = nvda.ts, 
                      n.ahead = length(nDate))
data.model.ramal <- model.ramal@forecast[["seriesFor"]]


#Data Frame
data_ramal <- data.frame(
  Date = seq(from = min(data2$Date),
             to = max(nDate)-1, by="days")  ,
  `Adj Close` = c(as.vector(nvda.ts), data.model.ramal[-1] ),
  Name = rep(c("NVDA"), 
              each = length(nvda.ts) + length(nDate)-1 )
)
...
# Akhir data
  ...
  #Label Data Asli 
  annotate( "rect", alpha=.1, fill="#4EC2C1",
            xmin=as.Date(min(data_ramal$Date)), 
            xmax=as.Date(data_ramal$Date[nrow(train_nvda)]),
            ymin=max(data_ramal$`Adj Close`) * .9, ymax=Inf ) + 
  
  annotate( "text", color="#4EC2C1",
            x = as.Date(data_ramal$Date[ceiling(nrow(train_nvda)/2)]), 
            y = max(data_ramal$`Adj Close`) * .95, 
    label = "Data Latih", size=10) + 
  annotate( "text", color="#4EC2C1",
            x = as.Date(data_ramal$Date[ceiling(nrow(train_nvda)/2)]), 
            y = max(data_ramal$`Adj Close`) * .85, 
    label = paste0(nrow(train_nvda)," Hari (", 
                   round(nrow(data_ramal)/length(nDate)*100, 1), 
                   "%)"), size=10) + 
  
  #Label Data Ramal
  annotate( "rect", alpha=.1, fill="violetred",
            xmin=as.Date(data_ramal$Date[nrow(train_nvda)]), 
            xmax=as.Date(max(data_ramal$Date)),
            ymin=max(data_ramal$`Adj Close`) * .9, ymax=Inf ) + 
  
  annotate( "text", color="violetred",
            x = as.Date(data_ramal$Date[ceiling( .89*length(data_ramal$Date)/7)]) , 
            y = max(data_ramal$`Adj Close`) * .95, 
    label = "Data Ramal", size=10) +
  annotate( "text", color="violetred",
            x = as.Date(data_ramal$Date[ceiling( .89*length(data_ramal$Date)/7)]) , 
  ...
  geom_vline( #Buat garis batas data
    xintercept = as.Date(data_ramal$Date[nrow(train_nvda)]) , 
             linetype = "dotted", color = "black", linewidth = 2) +
...
#Time Series
  ...
  #Data Ramal ARCH + Intersep
    #Dari data ramal saja
  geom_line(data = data_ramal[nrow(train_nvda):nrow(nvda),], linewidth=2,
  ...
  #Axis
    ...
    date_breaks = "1 year",  # Menampilkan label setiap tahun
    date_labels = "%Y",  # Format label tahun
    limits = c(as.Date(min(data_ramal$Date)), 
               as.Date(max(data_ramal$Date)) + 80)
  ) +
    scale_y_continuous( #Sumbu y
    labels = scales::dollar_format(prefix = "$") #tambahin dolar
  ) +
...
#Data Uji
  geom_segment(aes(x = as.Date(max(data_ramal$Date)), 
                   xend = as.Date(max(data_ramal$Date))+60, 
                   y = nvda$`Adj Close`[nrow(nvda)], 
                   yend = nvda$`Adj Close`[nrow(nvda)]), 
               arrow = arrow(type = "closed", length = unit(.1, "inches")), 
               lineend = "round", color = "gray60", size=1.5) +
  geom_richtext(
    data = data.frame(x = as.Date(max(data_ramal$Date))+60, 
                      y = nvda$`Adj Close`[nrow(nvda)], 
                      label = "Data Asli" ),
    aes(x, y, label = label), size = 8, color = "white", 
    fill = "gray60", box.color = "white", parse = TRUE
...
# Data Ramal ARIMA
  geom_segment(aes(x = as.Date(max(data_ramal$Date)), 
                   xend = as.Date(max(data_ramal$Date)) + 60, 
                   y = max(data_ramal$`ARIMA`), 
                   yend = max(data_ramal$`ARIMA`)), 
               arrow = arrow(type = "closed", length = unit(.1, "inches")), 
               lineend = "round", color = "#D02A49", size=1.5) +
  geom_richtext(
    data = data.frame(x = as.Date(max(data_ramal$Date)) +60, 
...
# Data Ramal ARCH + Intersep
  geom_segment(aes(x = as.Date(max(data_ramal$Date)), 
                   xend = as.Date(max(data_ramal$Date)) + 70, 
                   y = max(data_ramal$`ARCH+Intersep`), 
                   yend = max(data_ramal$`ARCH+Intersep`)), 
               arrow = arrow(type = "closed", length = unit(.1, "inches")), 
               lineend = "round", color = "#4B75BA", size=1.5) +
  geom_richtext(
    data = data.frame(x = as.Date(max(data_ramal$Date)) +70, 
                      y = max(data_ramal$`ARCH+Intersep`), 
                      label = "ARCH + Intersep" ),
    aes(x, y, label = label), size = 8, color = "white", 
    fill = "#4B75BA", box.color = "white", parse = TRUE
...
# Data Ramal ARCH - Intersep
  geom_segment(aes(x = as.Date(max(data_ramal$Date)), 
                   xend = as.Date(max(data_ramal$Date)) + 70, 
                   y = data_ramal$`ARCH-Intersep`[nrow(nvda)], 
                   yend = data_ramal$`ARCH-Intersep`[nrow(nvda)]), 
               arrow = arrow(type = "closed", length = unit(.1, "inches")), 
               lineend = "round", color = "#DC4F26", size=1.5) +
  geom_richtext(
    data = data.frame(x = as.Date(max(data_ramal$Date)) +70, 
