...
model.ramal = ugarchforecast(model.arch_nvda1, data = nvda.diff, 
                      n.ahead = length(nDate)+2)
data.model.ramal <- model.ramal@forecast[["seriesFor"]]

#--Diff inv--
  #ARIMA
  ramal.nvda <- diffinv(data.model.ramal, differences=1) +
    train_nvda.ts.new[length(train_nvda.ts)] #nilai akhir data latih


#--Transformasi Balik--
  ramal.nvda <- 1/(ramal.nvda)^2

#Data Frame
data_ramal <- data.frame(
  Date = seq(from = min(data2$Date),
             to = max(nDate), by="days")  ,
  `Adj Close` = c(as.vector(nvda.diff), data.model.ramal[-1] ),
  Name = rep(c("NVDA"), 
              each = length(nvda.diff) + length(nDate)+1 )
)
