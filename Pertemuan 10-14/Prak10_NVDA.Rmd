...
data.ramal_nvda.bb <- ramalan_nvda@forecast[["seriesFor"]] -
                          ramalan_nvda@forecast[["sigmaFor"]]
...
```{r fig.align="center", fig.height=6, fig.width=11, message=FALSE, warning=FALSE, dpi=300}
ts.plot(ramalan_nvda@forecast$sigmaFor)
ts.plot(ramalan_nvda@forecast$seriesFor)
```

```{r}
ggplot()+
    geom_line(data = data_ramal, linewidth=1,
            aes(x = Date, y = `Adj Close`, col = "NVDA")) +
  #BB

  #BA
  geom_line(data = data_ramal, linewidth=1,
            aes(x = Date, y = `BA`, col = "NVDA")) 
```
