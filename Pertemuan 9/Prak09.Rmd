...
Kategori Model terbaik **tidak ada** yang terpenuhi, yakni tidak ada nilai AIC terkecil dengan semua parameter nya signifikan dan tidak ada NA di dalam nya. Namun jika mengabaikan kategorinya, maka model terbaiknya adalah `ARIMA(4,1,4)` dengan nilai AIC sebesar $2137.52$.

```{r}
model_aapl.drift2 <- Arima(train_aapl.diff, order=c(4,1,4), method="ML")
```
...
```{r}
model_amzn.drift2 <- Arima(train_amzn.diff, order=c(2,1,2), method="ML")
```
...
```{r}
model_goog.drift2 <- Arima(train_goog.diff, order=c(2,1,2), method="ML")
```
...
```{r}
model_meta.drift2 <- Arima(train_meta.diff, order=c(2,1,2), method="ML")
```
...
```{r}
model_msft.drift2 <- Arima(train_msft.diff, order=c(2,1,2), method="ML")
```
...
```{r}
model_nflx.drift2 <- Arima(train_nflx.diff, order=c(2,1,2), method="ML")
```
...
```{r}
model_nvda.drift2 <- Arima(train_nvda.diff, order=c(2,1,2), method="ML")
```
