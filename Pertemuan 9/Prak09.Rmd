...
```{r fig.align=, fig.height=10, fig.width=20, message=FALSE, warning=FALSE, dpi=300}
install_load('tsibble','tseries')
par(mfrow=c(2,4))
acf(train_aapl.ts, main="AAPL"); acf(train_amzn.ts, main="AMZN")
acf(train_goog.ts, main="GOOG"); acf(train_meta.ts, main="META")
acf(train_msft.ts, main="MSFT"); acf(train_nflx.ts, main="NFLX")
acf(train_nvda.ts, main="NVDA")
```
...
bc =boxcox(train_nvda.ts~index, lambda=seq(-2, 4, by=.01)); title("NVDA")
...
Mengutip dari laman tersebut, formula transformasi data disesuaikan dengan nilai *rounded value* ($\lambda$) optimumnya.
...
```{r fig.align=, fig.height=10, fig.width=15, message=FALSE, warning=FALSE, dpi=300}
#Transformasi
...
par(mfrow=c(2,3))
index <- seq(1:nrow(train_aapl))
bc <-boxcox(train_amzn.ts.new ~index, lambda=seq(-5, 6, by=.01)); title("AMZN")
bc <-boxcox(train_goog.ts.new ~index, lambda=seq(-5, 6, by=.01)); title("GOOG")
bc <-boxcox(train_meta.ts.new ~index, lambda=seq(-5, 6, by=.01)); title("META")
bc <-boxcox(train_msft.ts.new ~index, lambda=seq(-5, 6, by=.01)); title("MSFT")
bc <-boxcox(train_nflx.ts.new ~index, lambda=seq(-5, 6, by=.01)); title("NFLX")
index <- seq(1:nrow(train_nvda))
bc <-boxcox(train_nvda.ts.new ~index, lambda=seq(-5, 6, by=.01)); title("NVDA")
```
