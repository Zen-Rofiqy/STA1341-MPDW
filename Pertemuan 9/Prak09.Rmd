...
Kategori Model terbaik **tidak ada** yang terpenuhi, yakni tidak ada nilai AIC terkecil dengan semua parameter nya signifikan dan tidak ada NA di dalam nya. Walaupun jika mengabaikan kategorinya, pemilihan modle terbaiknya agak cukup sulit. Hanya ada **tiga** model yang parameternya tidak memiliki **NA** di dalamnya. Merika adalah `ARIMA(1,0,12), ARIMA(4,0,11), dan ARIMA(2,0,12).`m Itupun mereka banyak parameternya yang signifikan. Diputuskanlah model terbaiknya adalah `ARIMA(``2,0,12``)` dengan nilai AIC yang lebih kecil daripada ketiganya yakni sebesar $-2297.46$.

```{r}
model_goog.drift4 <- Arima(train_goog.diff, order=c(2,1,2), method="ML",
                           include.drift=TRUE)
```
...
```{r}
model_meta.drift4 <- Arima(train_meta.diff, order=c(2,1,2), method="ML",
                           include.drift=TRUE)
```
...
```{r}
model_msft.drift4 <- Arima(train_msft.diff, order=c(2,1,2), method="ML",
                           include.drift=TRUE)
```
...
```{r}
model_nflx.drift4 <- Arima(train_nflx.diff, order=c(2,1,2), method="ML",
                           include.drift=TRUE)
```
...
```{r}
model_nvda.drift4 <- Arima(train_nvda.diff, order=c(2,1,2), method="ML",
                           include.drift=TRUE)
```
