...
```{r}
N <- nrow(train_aapl)
c(-1,1)*(exp(2*1.96/sqrt(N-3))-1)/(exp(2*1.96/sqrt(N-3))+1)

acf(train_aapl.diff, plot=F)[["lag"]]
acf(train_aapl.diff, plot=F)[["acf"]]

acf(train_aapl.diff, plot = F)[["n.used"]] + 1


acf.pacf <- function(dt){
  N <- acf(dt, plot = F)[["n.used"]] + 1
  ...
  cut.off <- merge(cut.off, cut.off2, by = 0, all = TRUE)[-1]
  return(cut.off)
}

acf.pacf_aapl <- acf.pacf(dt = train_aapl.diff) %>% 
  rename(AAPL.a=acf, AAPL.p=pacf)
acf.pacf_amzn <- acf.pacf(dt = train_amzn.diff) %>% 
  rename(AMZN.a=acf, AMZN.p=pacf)
acf.pacf_goog <- acf.pacf(dt = train_goog.diff) %>% 
  rename(GOOG.a=acf, GOOG.p=pacf)
acf.pacf_meta <- acf.pacf(dt = train_meta.diff) %>% 
  rename(META.a=acf, META.p=pacf)
acf.pacf_msft <- acf.pacf(dt = train_msft.diff) %>% 
  rename(MSFT.a=acf, MSFTL.p=pacf)
acf.pacf_nflx <- acf.pacf(dt = train_nflx.diff) %>% 
  rename(NFLX.a=acf, NFLX.p=pacf)
acf.pacf_nvda <- acf.pacf(dt = train_nvda.diff) %>% 
  rename(NVDA.a=acf, NVDA.p=pacf)

install_load('data.table')

acf.pacf.tab <- (cbind(acf.pacf_aapl, acf.pacf_amzn, acf.pacf_goog, 
                      acf.pacf_meta, acf.pacf_msft, acf.pacf_nflx, 
                      acf.pacf_nvda))
```
